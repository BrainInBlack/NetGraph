...
The detail panel (`src/ts/ui/sidebar.ts openPanel`) uses the same hash trick (`panelContentHash`) so dragging a selected device doesn't churn the side panel innerHTML 60×/sec.
